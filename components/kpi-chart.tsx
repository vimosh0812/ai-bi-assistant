...
export function KPIChart({ 
  ...
  // Helper function to generate individual colors
  const getIndividualColors = (count: number): string[] => {
    const baseColors = ['#3B82F6', '#10B981', '#F59E0B', '#EF4444', '#8B5CF6', '#EC4899', '#06B6D4', '#84CC16'];
    const colors: string[] = [];
  ...
  // Helper function to get current colors being used
  const getCurrentColors = (): string[] => {
    if (customColors.length > 0) {
      return customColors;
    }
    if (finalChartData && finalChartData.datasets && finalChartData.datasets[0]) {
      const bgColor = finalChartData.datasets[0].backgroundColor;
  ...
  const colorPalettes: Record<string, { bar: string[]; pie: string[] }> = {
    blue: {
      bar: ['#3B82F6', '#2563EB', '#1D4ED8', '#1E40AF', '#1E3A8A', '#172554'],
      pie: ['#3B82F6', '#2563EB', '#1D4ED8', '#1E40AF', '#1E3A8A', '#172554', '#0F172A', '#1E293B']
    },
    green: {
      bar: ['#10B981', '#059669', '#047857', '#065F46', '#064E3B', '#022C22'],
      pie: ['#10B981', '#059669', '#047857', '#065F46', '#064E3B', '#022C22', '#ECFDF5', '#D1FAE5']
    },
    purple: {
      bar: ['#8B5CF6', '#7C3AED', '#6D28D9', '#5B21B6', '#4C1D95', '#3B1F6F'],
      pie: ['#8B5CF6', '#7C3AED', '#6D28D9', '#5B21B6', '#4C1D95', '#3B1F6F', '#2E1B4D', '#1E0F3D']
    },
    orange: {
      bar: ['#F59E0B', '#D97706', '#B45309', '#92400E', '#78350F', '#451A03'],
      pie: ['#F59E0B', '#D97706', '#B45309', '#92400E', '#78350F', '#451A03', '#FEF3C7', '#FDE68A']
    },
    red: {
      bar: ['#EF4444', '#DC2626', '#B91C1C', '#991B1B', '#7F1D1D', '#450A0A'],
      pie: ['#EF4444', '#DC2626', '#B91C1C', '#991B1B', '#7F1D1D', '#450A0A', '#FEE2E2', '#FECACA']
    }
  };
            ...
            <div className="space-y-4">
              ...
              {!showIndividualColors ? (
                ...
                <div className="space-y-3">
                  {Object.entries(colorPalettes).map(([paletteName, palette]: [string, { bar: string[]; pie: string[] }]) => (
                    <div key={paletteName} className="space-y-2">
                      <div className="text-xs font-medium capitalize text-gray-600">
                        {paletteName} Palette
                      </div>
                      <div className="flex flex-wrap gap-1">
                        {palette[currentPalette].slice(0, 8).map((color: string, index: number) => (
                          <button
                            key={index}
                  ...
                  {finalChartData && finalChartData.labels && finalChartData.labels.map((label: string, index: number) => (
                    <div key={index} className="flex items-center justify-between p-2 bg-gray-50 rounded">
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          setIndividualColors((prev: Record<number, string>) => ({
                            ...prev,
